...
def parse_args() -> argparse.Namespace:
    ...
    if args.yolo_only:
        args.use_yolo = True
        args.use_pose = args.use_megadetector = args.use_frcnn = False
    else:
...
def process_image(image_path: Path, models: dict, confidence_threshold: float, min_confidence: float) -> tuple:
...
def process_image_megadetector(model: MegaDetectorV6, image_path: Path, min_confidence: float) -> list:
    ...
    return [conf for conf, class_id in zip(detections.confidence, detections.class_id) 
            if class_id == 1]  # class 1 is person in MegaDetector
...
def process_image_frcnn(model: torch.nn.Module, image_path: Path, min_confidence: float) -> list:
    ...
    return [score.item() for label, score in zip(prediction[0]['labels'], prediction[0]['scores'])
            if label == 1 and score >= min_confidence]  # class 1 is person in COCO

def ensure_output_dir(count: int, timestamp: str, uncertain: bool = False, models: dict = None, base_path: str = 'output') -> Path:
    ...
    for image_path in input_path.glob(glob_pattern):
        try:
            ...
            if is_uncertain:
                ...
                dest_dir = ensure_output_dir(count, timestamp, uncertain=True, models=models, base_path=args.output_path)
                shutil.copy2(image_path, dest_dir / image_path.name)
                print(f"Copied to {dest_dir}/")
            elif count > 0:
                print(f"Found {count} people in {image_path.name}")
                dest_dir = ensure_output_dir(count, timestamp, uncertain=False, models=models, base_path=args.output_path)
                shutil.copy2(image_path, dest_dir / image_path.name)
                print(f"Copied to {dest_dir}/")
            else:
